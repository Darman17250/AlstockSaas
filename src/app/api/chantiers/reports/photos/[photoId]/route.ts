import { NextResponse } from 'next/server'

import { NotFoundError, UnauthorizedError, requireOrgContext } from '@/lib/auth/org-context'
import { StorageNotConfiguredError } from '@/lib/supabase-storage'
import { getSiteReportPhotoDownload } from '@/services/crm/site-report-photo'

export const dynamic = 'force-dynamic'

/** Affichage d'une photo de rapport : redirige vers une URL signée temporaire. */
export async function GET(_request: Request, { params }: { params: Promise<{ photoId: string }> }) {
  const { photoId } = await params
  try {
    const ctx = await requireOrgContext()
    const { url } = await getSiteReportPhotoDownload(ctx, photoId)
    return NextResponse.redirect(url)
  } catch (e) {
    if (e instanceof UnauthorizedError)
      return NextResponse.json({ error: 'Authentification requise' }, { status: 401 })
    if (e instanceof NotFoundError) return NextResponse.json({ error: e.message }, { status: 404 })
    if (e instanceof StorageNotConfiguredError)
      return NextResponse.json({ error: e.message }, { status: 503 })
    console.error('download site report photo failed', e)
    return NextResponse.json({ error: 'Échec du téléchargement' }, { status: 500 })
  }
}
