...
import { getSiteDocumentDownload } from '@/services/crm/site-document'

export const dynamic = 'force-dynamic'

/**
 * Téléchargement d'un document de chantier : redirige vers une URL signée
 * temporaire (bucket privé). `?download=1` force le téléchargement avec le nom d'origine.
 */
export async function GET(request: Request, { params }: { params: Promise<{ docId: string }> }) {
  const { docId } = await params
  try {
    const ctx = await requireOrgContext()
    const { url, fileName } = await getSiteDocumentDownload(ctx, docId)
    ...
    console.error('download site document failed', e)
