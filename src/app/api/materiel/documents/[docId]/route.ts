...
import { getToolDocumentDownload } from '@/services/crm/tool-document'

export const dynamic = 'force-dynamic'

/**
 * Téléchargement/affichage d'un document de matériel : redirige vers une URL
...
export async function GET(request: Request, { params }: { params: Promise<{ docId: string }> }) {
  ...
  try {
    ...
    const { url, fileName } = await getToolDocumentDownload(ctx, docId)
    ...
    console.error('download tool document failed', e)
