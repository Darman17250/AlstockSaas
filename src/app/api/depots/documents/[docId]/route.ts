...
import { getDepotDocumentDownload } from '@/services/crm/depot-document'

export const dynamic = 'force-dynamic'

/**
 * Téléchargement/affichage d'un document de dépôt : redirige vers une URL signée
...
export async function GET(request: Request, { params }: { params: Promise<{ docId: string }> }) {
  ...
  try {
    ...
    const { url, fileName } = await getDepotDocumentDownload(ctx, docId)
    ...
    console.error('download depot document failed', e)
