...
import { getEquipmentDocumentDownload } from '@/services/crm/equipment-document'

export const dynamic = 'force-dynamic'

/**
 * Téléchargement/affichage d'un document : redirige vers une URL signée
 * temporaire (bucket privé). `?download=1` force le téléchargement.
 */
export async function GET(request: Request, { params }: { params: Promise<{ docId: string }> }) {
  const { docId } = await params
  try {
    const ctx = await requireOrgContext()
    const { url, fileName } = await getEquipmentDocumentDownload(ctx, docId)
    ...
    console.error('download equipment document failed', e)
