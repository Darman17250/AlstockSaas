...
import type { DealDocumentItem } from '@/services/crm/deal-document'
import { DOCUMENT_ACCEPT } from '@/validation/deal-document'
import { deleteDealDocumentAction } from '../../actions'
...
export const DocumentsSection = ({
  ...
  const handleFiles = async (files: FileList | null) => {
    ...
    try {
      for (const file of Array.from(files)) {
        ...
        fd.append('file', file)
        const res = await fetch(`/api/affaires/${dealId}/documents`, {
          method: 'POST',
          body: fd,
        })
        if (!res.ok) {
  ...
  const handleDelete = async (documentId: string) => {
    ...
    setError(null)
    const res = await deleteDealDocumentAction(documentId, dealId)
    setDeletingId(null)
          ...
          {documents.map((doc) => {
            ...
            return (
              <li key={doc.id} className='flex items-center gap-3 py-2'>
                ...
                <a
                  href={`/api/affaires/documents/${doc.id}?download=1`}
                  target='_blank'
                  rel='noreferrer'
