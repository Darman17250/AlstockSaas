...
import type { EquipmentDocumentItem } from '@/services/crm/equipment-document'
import { DOCUMENT_ACCEPT } from '@/validation/deal-document'
import { deleteEquipmentDocumentAction } from '../../actions'
...
export const EquipmentDocumentsSection = ({
  equipmentId,
  documents,
  canEdit,
  storageConfigured,
  ...
  const handleFiles = async (files: FileList | null) => {
    ...
    try {
      for (const file of Array.from(files)) {
        ...
        fd.append('file', file)
        const res = await fetch(`/api/equipements/${equipmentId}/documents`, {
          method: 'POST',
          body: fd,
        })
        if (!res.ok) {
  ...
  const handleDelete = async (documentId: string) => {
    ...
    setError(null)
    const res = await deleteEquipmentDocumentAction(documentId, equipmentId)
    setDeletingId(null)
  ...
  return (
    <section className='space-y-3 rounded-lg border p-5'>
      <div className='flex items-center justify-between gap-3'>
        <h2 className='font-semibold'>Documents & photos</h2>
      ...
      {documents.length === 0 ? (
        storageConfigured && (
          <p className='text-sm text-muted-foreground'>
            Aucun document. Photos, plaque signalétique, manuel PDF, factures…
          </p>
        )
      ) : (
        <ul className='divide-y'>
          {documents.map((doc) => {
            ...
            const inlineUrl = `/api/equipements/documents/${doc.id}`
            return (
