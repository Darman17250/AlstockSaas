...
import { createToolReminderTaskAction } from '../../actions'

interface ToolReminderButtonProps {
  toolId: string
  subject: string
  dueDate: string
}
...
export const ToolReminderButton = ({ toolId, subject, dueDate }: ToolReminderButtonProps) => {
  ...
  const handleClick = async () => {
    setBusy(true)
    setError(null)
    const res = await createToolReminderTaskAction({ toolId, subject, dueDate })
    setBusy(false)
    if (!res.ok) {
      setError(res.error)
      return
