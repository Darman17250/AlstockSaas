...
import type { LibraryCategoryWithSubcategories } from '@/services/admin/library'
import {
  createLibraryCategoryAction,
  createLibrarySubcategoryAction,
  deleteLibraryCategoryAction,
  deleteLibrarySubcategoryAction,
  updateLibraryCategoryAction,
  updateLibrarySubcategoryAction,
} from '../actions'

interface LibraryCategoriesManagerProps {
  categories: LibraryCategoryWithSubcategories[]
}
...
export const LibraryCategoriesManager = ({ categories }: LibraryCategoriesManagerProps) => {
  ...
  const submit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (dialog.kind === 'category') {
      res = dialog.id
        ? await updateLibraryCategoryAction(dialog.id, { name: trimmed })
        : await createLibraryCategoryAction({ name: trimmed })
    } else {
      res = dialog.id
        ? await updateLibrarySubcategoryAction(dialog.id, { name: trimmed })
        : await createLibrarySubcategoryAction({ categoryId: dialog.categoryId, name: trimmed })
    }
  ...
  const remove = async (kind: 'category' | 'subcategory', id: string) => {
    const res =
      kind === 'category'
        ? await deleteLibraryCategoryAction(id)
        : await deleteLibrarySubcategoryAction(id)
    if (!res.ok) {
    ...
    <div className='space-y-4'>
      ...
      {categories.length === 0 ? (
        <div className='flex flex-col items-center gap-2 rounded-lg border border-dashed p-8 text-center text-muted-foreground'>
          ...
          <p className='text-sm'>Aucune catégorie dans le catalogue.</p>
        </div>
      ) : (
